...
data_AP1_family <- data_AP1_all[data_AP1_all$tissue == "Spleen",]
...
write.table(df_AP1_family_all, "Spleen_FC_AP1_all.txt", sep="\t", quote=FALSE)
...
Jun_B6_test <- t.test(data_Jun_B6$FC, mu=1, alternative = "greater")
Jun_NZO_test <- t.test(data_Jun_NZO$FC, mu=1, alternative = "greater")
Fos_B6_test <- t.test(data_Fos_B6$FC, mu=1, alternative = "greater")
Fos_NZO_test <- t.test(data_Fos_NZO$FC, mu=1, alternative = "greater")
Atf_B6_test <- t.test(data_Atf_B6$FC, mu=1, alternative = "greater")
Atf_NZO_test <- t.test(data_Atf_NZO$FC, mu=1, alternative = "greater")
Maf_B6_test <- t.test(data_Maf_B6$FC, mu=1, alternative = "greater")
Maf_NZO_test <- t.test(data_Maf_NZO$FC, mu=1, alternative = "greater")
...
Jun_B6_wilcox_test <- wilcox.test(data_Jun_B6$FC, mu=1, alternative = "greater")
Jun_NZO_wilcox_test <- wilcox.test(data_Jun_NZO$FC, mu=1, alternative = "greater")
Fos_B6_wilcox_test <- wilcox.test(data_Fos_B6$FC, mu=1, alternative = "greater")
Fos_NZO_wilcox_test <- wilcox.test(data_Fos_NZO$FC, mu=1, alternative = "greater")
Atf_B6_wilcox_test <- wilcox.test(data_Atf_B6$FC, mu=1, alternative = "greater")
Atf_NZO_wilcox_test <- wilcox.test(data_Atf_NZO$FC, mu=1, alternative = "greater")
Maf_B6_wilcox_test <- wilcox.test(data_Maf_B6$FC, mu=1, alternative = "greater")
Maf_NZO_wilcox_test <- wilcox.test(data_Maf_NZO$FC, mu=1, alternative = "greater")
...
pdf("Boxplot_Spleen_FC_RNA_DE_not_filtered_all_24_AP1_genes.pdf", height=10, width=10)
p
dev.off()

#ggplot(df_AP1_family_all, aes(x=AP1_subfamily, y = FC, fill=strain)) + geom_boxplot() + facet_wrap(~tissue, scale = "free", nrow=1) + theme_pubr(base_size=12) + stat_compare_means(method = "t.test")
#ggplot(df_AP1_family_all, aes(x=AP1_subfamily, y = FC, fill=strain)) + geom_boxplot() + geom_jitter(color="black", size=0.8, alpha = 0.9) + facet_wrap(~tissue, scale = "free", nrow=1) + theme_pubr(base_size=12)
#pdf("Boxplot_Spleen_wilcox_FC_RNA_DE_not_filtered_all_24_AP1_genes.pdf", width=15)
#ggplot(df_AP1_family_all, aes(x=AP1_subfamily, y = FC, fill=strain)) + geom_boxplot() + facet_wrap(~tissue, scale = "free", nrow=1) + theme_pubr(base_size=12) + stat_compare_means(method = "wilcox")
#dev.off()
