...
DE_fit_quantile_batch_corrected <-
    function(bed, bed.normalized, specs) {
        ...
        sva.res <-
            sva::svaseq(bed.normalized , design, mod0)
...
DE_annotate_genes_GSEA <- function (top.table, meta, homer_table, tss_cutoff = 10e3) {
...
DE_annotate_genes <- function (top.table, meta, homer_table, 
                               tss_cutoff = 10e3, logFC.cutoff = 0) {
  # let's say we compare age, old vs young, then if log2 fold change for a peak is bigger than one, it means that the peak in old ones is expressed at least two times more compared to that peak in young ones. So, they are opening peaks in older ones.
...
preprocess_data <- function(tissue_cell_type, exp_type="ATAC"){
  if (exp_type == "ATAC"){
    ...
    # normalize with cpm
    bed.normalized <- cpm(bed.filtered, log = F)
    ...
    for (var in unique(df$geneset.name)){
      p <- ggplot(df[df$geneset.name == var,],
        aes(
          ...
          y = Contrast,
          color = Status,
          size = ifelse(adj.p < 0.25,-log10(adj.p), NA)
        )) +
          xlab("") + ylab("") +
          geom_point() +
          coord_flip() +
...
draw_bar_plots <- function(all_bar_plots){
  ...
  df_all_peaks$TCT[df_all_peaks$TCT == "naive" ] <- "Naive"
  df_all_peaks$TCT[df_all_peaks$TCT == "memory"] <- "Memory"
  ...
  ggsave(paste0('output/F4/Gene and Peak Counts/Age18vs3_peak_counts_cell_types.pdf'),
  ...
  df_all_genes$TCT[df_all_genes$TCT == "naive"] <- "Naive"
  df_all_genes$TCT[df_all_genes$TCT == "memory"] <- "Memory"
  ...
  plot.genes.cell.types <- ggplot(df_all_genes %>%
    ...
    geom_text(aes(
      y = lab_ypos,
      label = ifelse(as.numeric(Gene.Count) < 1, "",
  ...
  ggsave(
    paste0(
      'output/F4/Gene and Peak Counts/Atacseq_18x3_genecounts_cell_types.pdf'
    ),
...
run_tissue <- function(tissue_cell_type, fit_cache = NULL){
  ...
  if (is.null(fit_cache)){
    fit <- DE_fit_quantile_batch_corrected(bed, bed.normalized, specs)
  } else {
  ...
  er_modules_list <- lapply(contrasts, function(contrast, tissue_cell_type, meta, homer_table){
    ...
    write.csv(x =  top.table.GSEA,
          file = paste0("output/F4/DE_Genes/Batch_Corrected_ALL_genes_", 
    ...
    if(nrow(top.table) > 0){
      ...
      write.csv(x =  atac.gene.file,
                file = paste0("output/F4/DE_Genes/Batch_Corrected_DE_genes_", 
                              toupper(tissue_cell_type), "_", 
                              contrast, "_ATACseq.csv"))
  ...
  # Save file
  filename_er_modules <- paste0("output/F4/Enrichment Files/Hypergeometric/ATACseq/",
  ...
  # Make leadingE dge gene list character csv
  gsea.modules$leadingEdge <- sapply(gsea.modules$leadingEdge,function(x){paste(x,collapse = ",")})
  filename_gsea_modules <- paste0("output/F4/Enrichment Files/GSEA/ATACseq/",
...
draw_bar_plots <- function(all_bar_plots){
  ...
  ggsave(paste0('output/F4/Gene and Peak Counts/Age18vs3_peak_counts_tissues.pdf'),
  ...
  plot.genes.tissues <- ggplot(df_all_genes, 
                               aes(x = TCT, y = as.numeric(Gene.Count))) +
    geom_col(aes(fill = Reg), width = 0.5) + 
    geom_text(aes(y = lab_ypos, label = ifelse(as.numeric(Gene.Count) < 1000, "", as.numeric(Gene.Count)), 
  ...
  ggsave(paste0('output/F4/Gene and Peak Counts/Atacseq_18x3_genecounts_tissues.pdf'),
...
wp.labels <- selected_genesets_labels[["wp"]]
wp.inflammation <-wp[wp$Module.ID %in% unique(wp.inflammation$Module.ID),]

wp.merged <- merge(wp, wp.labels, "Module.ID")
wp.merged <- wp.merged[wp.merged$Module.ID %in% wp.inflammation$Module.ID,]

selected_genesets_mice[["wp_inflammation"]] <- wp.inflammation
selected_genesets_gsea[["wp_inflammation"]] <- split(wp.merged$Gene.stable.ID, wp.merged$Module.Name)



fit_cache <- lapply(list.cell_type, function(x){run_tissue(x)})

all_bar_plots <- lapply(list.cell_type, function(tissue_cell_type, fit_cache){
  create_bar_plots(tissue_cell_type, fit_cache)
}, fit_cache) %>% invisible
names(all_bar_plots) <- list.cell_type
draw_bar_plots(all_bar_plots)


# Hypergeometric Enrichment Figures
er_plot("output/F4/Enrichment Files/Hypergeometric/ATACseq/", 
        "output/F4/Enrichment Figures/ATACseq/", gsea = F)


er_plot("output/F4/Enrichment Files/GSEA/ATACseq/", 
        "output/F4/Enrichment Figures/ATACseq/", gsea = T)
```
